...
# (up, down, left, right or diagonally) in the following 20x20 grid?
#
...
# the grid on which we will be working
grid20by20 = [[8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 04, 05, 07, 78, 52, 12, 50,
               77, 91, 8],
...
sizeGrid = len(grid20by20)


###############################################################################
# FUNCTIONS CALCULATING THE GREATEST PRODUCTS
###############################################################################
def greatestProductHorizontal(grid20by20):
    res = 0
    for row in grid20by20:
        for j in len(row)-4:
            tmp = row[j]*row[j+1]*row[j+2]*row[j+3]
            if tmp > res:
                res = tmp
    return res


def greatestProductVertical(grid20by20):
    res = 0
    for j in sizeGrid:
        for i in sizeGrid-4:
            tmp = (grid20by20[i][j] * grid20by20[i+1][j] * grid20by20[i+2][j]
                   * grid20by20[i+3][j])
            if tmp > res:
                res = tmp
    return res
...
# XXX To continue...
def greatestProductDiagonalFromLeftToRight(grid20by20):
    res = 0
    return False
